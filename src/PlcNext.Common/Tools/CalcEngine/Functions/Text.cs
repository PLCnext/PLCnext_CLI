...
namespace CalcEngine
{
    internal static class Text
    {
        ...
        {
            return ((string)p[0]).ToLowerInvariant();
        }
        ...
        static object Substitute(List<Expression> p)
        {
            ...
            int pos = text.IndexOf(oldText, StringComparison.Ordinal);
            while (pos > -1 && index > 1)
            {
        ...
        {
            return ((double)p[0]).ToString(p[1], CultureInfo.CurrentCulture);
        }
        ...
        {
            return ((string)p[0]).ToUpper(CultureInfo.InvariantCulture);
        }
